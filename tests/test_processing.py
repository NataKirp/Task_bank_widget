...
def test_filter_by_state(
    transactions_id_state_date,
    transactions_state_executed,
    transactions_state_canceled,
    state,
):
    assert (
        filter_by_state(transactions_id_state_date, "EXECUTED")
        == transactions_state_executed
    )
    assert filter_by_state(transactions_id_state_date) == transactions_state_executed
    assert (
        filter_by_state(transactions_id_state_date, "CANCELED")
        == transactions_state_canceled
    )
...
def test_sort_by_date(
    ...
    assert (
        sort_by_date(transactions_id_state_date, descending=True)
        == sort_by_date_descending
    )
    assert (
        sort_by_date(transactions_id_state_date, descending=False)
        == sort_by_date_ascending
    )
    assert sort_by_date(transactions_id_state_date) == sort_by_date_descending
