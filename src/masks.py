...
def get_mask_account(account_number: Union[int, str]) -> str:
    ...
    # Проверка, что номер счета корректный и состоит из ACC_NUMBER_MIN_LENGTH цифр
    if len(account_number) != ACC_NUMBER_LENGTH or not account_number.isdigit():
        raise ValueError(
            "Неверный номер счета: '{account_number}'. Ожидаемый формат: {ACCOUNT_NUMBER_MIN_LENGTH} цифр."
        )
