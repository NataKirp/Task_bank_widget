...
def log(filename: Optional[str] = None) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
    ...
    def wrapper(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def inner(*args: Any, **kwargs: Any):
            ...
            result_text = 'ok' if text_error == '' else text_error
            log_text = (f"Начало выполнения функции {func.__name__}: {formatted_start_time}\n"
                        f"Конец выполнения функции {func.__name__}: {formatted_end_time}\n"
                        f"Результат выполнения функции {func.__name__}: {result_text}. Inputs: {args}, {kwargs}")
            if filename is None:
                print(log_text)
            else:
